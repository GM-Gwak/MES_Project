...
namespace test_bom
{
    public partial class Form1 : Form
    {
        ...
        private void start_1_Click(object sender, EventArgs e)
        {
            ...
            cmd.CommandText = $"update workorder set wostarttime = to_char(sysdate, 'yyyy-mm-dd hh24:mi:ss'), wostat = 'S' where woid = '{WC001.SelectedRows[0].Cells[1].Value.ToString()}'";
            cmd.ExecuteNonQuery();

            // 선택한 행 업데이트
            cmd.CommandText = $"update lot set lotstarttime = to_char(sysdate, 'yyyy-mm-dd hh24:mi:ss'), wcid = 'wc001', lotstat = 'S' where lotid = '{WC001.SelectedRows[0].Cells[0].Value.ToString()}'";
            cmd.ExecuteNonQuery();
            ...
            next_lotid = WC001.SelectedRows[0].Cells[0].Value.ToString();
        ...
        {
            // 선택한 행 업데이트
            cmd.CommandText = $"update lot set lotstarttime = to_char(sysdate, 'yyyy-mm-dd hh24:mi:ss'), wcid = 'wc002', lotstat = 'S' where lotid = '{WC002.SelectedRows[0].Cells[0].Value.ToString()}'";
            cmd.ExecuteNonQuery();

            // 선택한 행의 계획수량, PMID, WOID를 저장
            cmd.CommandText = $"select W.PMID from Workorder W, LOT L where W.WOID = L.WOID and W.WOID = '{WC002.SelectedRows[0].Cells[1].Value.ToString()}' and rownum = 1";
            rdr = cmd.ExecuteReader();
            rdr.Read();
            next_order_pmid = rdr["PMID"] as string;
            next_order_woid = WC002.SelectedRows[0].Cells[1].Value.ToString();
            next_order_planqty = Int32.Parse(WC002.SelectedRows[0].Cells[6].Value.ToString());
            next_lotid = WC002.SelectedRows[0].Cells[0].Value.ToString();
        ...
        {
            // 선택한 행 업데이트
            cmd.CommandText = $"update lot set lotstarttime = to_char(sysdate, 'yyyy-mm-dd hh24:mi:ss'), wcid = 'wc003', lotstat = 'S' where lotid = '{WC003.SelectedRows[0].Cells[0].Value.ToString()}'";
            cmd.ExecuteNonQuery();

            // 선택한 행의 계획수량, PMID, WOID를 저장
            cmd.CommandText = $"select W.PMID from Workorder W, LOT L where W.WOID = L.WOID and W.WOID = '{WC003.SelectedRows[0].Cells[1].Value.ToString()}' and rownum = 1";
            rdr = cmd.ExecuteReader();
            rdr.Read();
            next_order_pmid = rdr["PMID"] as string;
            next_order_woid = WC003.SelectedRows[0].Cells[1].Value.ToString();
            next_order_planqty = Int32.Parse(WC003.SelectedRows[0].Cells[6].Value.ToString());
            next_lotid = WC003.SelectedRows[0].Cells[0].Value.ToString();
        ...
        {
            // 선택한 행 업데이트
            cmd.CommandText = $"update lot set lotstarttime = to_char(sysdate, 'yyyy-mm-dd hh24:mi:ss'), wcid = 'wc004', lotstat = 'S' where lotid = '{WC004.SelectedRows[0].Cells[0].Value.ToString()}'";
            cmd.ExecuteNonQuery();

            // 선택한 행의 계획수량, PMID, WOID를 저장
            cmd.CommandText = $"select W.PMID from Workorder W, LOT L where W.WOID = L.WOID and W.WOID = '{WC004.SelectedRows[0].Cells[1].Value.ToString()}' and rownum = 1";
            rdr = cmd.ExecuteReader();
            rdr.Read();
            next_order_pmid = rdr["PMID"] as string;
            next_order_woid = WC004.SelectedRows[0].Cells[1].Value.ToString();
            next_order_planqty = Int32.Parse(WC004.SelectedRows[0].Cells[6].Value.ToString());
            next_lotid = WC004.SelectedRows[0].Cells[0].Value.ToString();
        ...
        private void end_4_Click(object sender, EventArgs e)
        {
            ...
            cmd.CommandText = $"update workorder set woendtime = to_char(sysdate, 'yyyy-mm-dd hh24:mi:ss'), wostat = 'E' where woid = '{next_order_woid}'";
            cmd.ExecuteNonQuery();
        }
